...
def run_inference(args):
    ...
    for batch in tqdm(dataloader, disable=not is_main_process):
        ...
        # PCK-only inference path:
        # Run only 2D branch (backbone -> keypoint_head), skipping 3D matching/evaluation logic.
        dino_features = model.backbone(images)
        pred_heatmaps = model.keypoint_head(dino_features)
